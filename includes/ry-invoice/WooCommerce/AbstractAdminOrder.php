...
abstract class AbstractAdminOrder
{
    ...
    public function bulk_action_notices()
    {
        ...
        if ($bulk_action === 'ry_get_invoice') {
            ...
            /* translators: %s: count */
            $message = sprintf(_n('%s order issue invoice.', '%s orders issue invoice.', $number, 'ry-invoice-for-amego'), number_format_i18n($number));
    ...
    public function add_invoice_column($columns)
    {
        if (!isset($columns['invoice-number'])) {
            $add_columns = [
                'invoice-number' => __('Invoice number', 'ry-invoice-for-amego'),
            ];
        ...
        if ($column === 'invoice-number') {
            ...
            if (!empty($invoice_number)) {
                match ($invoice_number) {
                    'wait' => esc_html_e('Wait get invoice', 'ry-invoice-for-amego'),
                    'zero' => esc_html_e('Zero no invoice', 'ry-invoice-for-amego'),
                    'negative' => esc_html_e('Negative no invoice', 'ry-invoice-for-amego'),
    ...
    {
        $actions['ry_get_invoice'] = __('Issue invoice', 'ry-invoice-for-amego');
    ...
    protected function get_fields($order)
    {
        ...
        $fields = [
            'type' => [
                'label' => __('Invoice type', 'ry-invoice-for-amego'),
                'show' => false,
                'class' => 'select short',
                'type' => 'select',
                'options' => [
            ...
            'no' => [
                'label' => __('Tax ID number', 'ry-invoice-for-amego'),
                'show' => false,
                'type' => 'text',
            ],
            'donate_no' => [
                'label' => __('Donate number', 'ry-invoice-for-amego'),
                'show' => false,
                'type' => 'text',
            ],
        ];
        ...
        if ($order->is_paid()) {
            $fields['number'] = [
                'label' => __('Invoice number', 'ry-invoice-for-amego'),
                'show' => false,
                'type' => 'text',
            ];
            $fields['random_number'] = [
                'label' => __('Invoice random number', 'ry-invoice-for-amego'),
                'show' => false,
                'type' => 'text',
                'pattern' => '[0-9]{4}',
            ];
            $fields['date'] = [
                'label' => __('Invoice date', 'ry-invoice-for-amego'),
                'show' => false,
                'type' => 'date',
            ];
        }
...
<h3 style="clear:both">
    <?php esc_html_e('Invoice info', 'ry-invoice-for-amego'); ?>
</h3>
    ...
    <div class="ivoice_data_column">
        <p>
            ...
            <?php switch ($invoice_number) {
                case 'wait': ?>
            <strong><?php esc_html_e('Invoice number', 'ry-invoice-for-amego'); ?>:</strong> <?php esc_html_e('Wait get invoice', 'ry-invoice-for-amego'); ?><br>
            <?php if ($scheduled_time > 0) {
                $scheduled_time = as_get_datetime_object($scheduled_time)->setTimezone(wp_timezone()); ?>
            <strong><?php esc_html_e('Expected get time', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($scheduled_time->format('Y-m-d H:i')); ?><br>
            <?php } ?>
            <?php
                    break;
                case 'zero': ?>
            <strong><?php esc_html_e('Invoice number', 'ry-invoice-for-amego'); ?>:</strong> <?php esc_html_e('Zero no invoice', 'ry-invoice-for-amego'); ?><br>
            <?php
                    break;
                case 'negative': ?>
            <strong><?php esc_html_e('Invoice number', 'ry-invoice-for-amego'); ?>:</strong> <?php esc_html_e('Negative no invoice', 'ry-invoice-for-amego'); ?><br>
            <?php
                    break;
                default: ?>
            <strong><?php esc_html_e('Invoice number', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($invoice_number); ?><br>
            <strong><?php esc_html_e('Invoice random number', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($order->get_meta('_invoice_random_number')); ?><br>
            <strong><?php esc_html_e('Invoice date', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($order->get_meta('_invoice_date')); ?><br>
            <?php
                    break;
            } ?>
            <?php } ?>
            ...
            <strong><?php esc_html_e('Carrier type', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html(Utils::carruer_type_to_name($carruer_type)); ?><br>

            <?php if (in_array($carruer_type, ['MOICA', 'phone_barcode'])) { ?>
            <strong><?php esc_html_e('Carrier number', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($order->get_meta('_invoice_carruer_no')); ?><br>
            <?php } ?>
            <?php } ?>

            <?php if ($invoice_type === 'company') { ?>
            <strong><?php esc_html_e('Tax ID number', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($order->get_meta('_invoice_no')); ?><br>
            <?php } ?>

            <?php if ($invoice_type === 'donate') { ?>
            <strong><?php esc_html_e('Donate number', 'ry-invoice-for-amego'); ?>:</strong> <?php echo esc_html($order->get_meta('_invoice_donate_no')); ?><br>
            <?php } ?>
        </p>
    </div>
    <div class="ivoice_action_column">
        ...
        if (preg_match('/^[A-Z]{2}[0-9]{8}$/', $invoice_number)) {
            echo '<button type="button" class="button ajax-' . esc_attr($this->type) . '-invoice" data-action="invalid" data-orderid="' . esc_attr($order->get_id()) . '">'
                . esc_html__('Invalid invoice', 'ry-invoice-for-amego')
                . '</button>';
            ...
            echo '<button type="button" class="button ajax-' . esc_attr($this->type) . '-invoice" data-action="cancel" data-orderid="' . esc_attr($order->get_id()) . '">'
                . esc_html__('Cancel get', 'ry-invoice-for-amego')
                . '</button>';
            ...
            echo '<button type="button" class="button ajax-' . esc_attr($this->type) . '-invoice" data-action="get" data-orderid="' . esc_attr($order->get_id()) . '">'
                . esc_html__('Issue invoice', 'ry-invoice-for-amego')
                . '</button>';
        }
    ?>
    </div>
...
<div class="edit_address">
    ...
    <p class="form-field form-field-wide <?php echo esc_attr($field['id']); ?>_field">
        ...
        &lrm;
        <input type="number" class="hour" placeholder="<?php esc_attr_e('h', 'ry-invoice-for-amego'); ?>" name="<?php echo esc_attr($field['id']); ?>_hour" min="0" max="23" step="1" value="" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:
        <input type="number" class="minute" placeholder="<?php esc_attr_e('m', 'ry-invoice-for-amego'); ?>" name="<?php echo esc_attr($field['id']); ?>_minute" min="0" max="59" step="1" value="" pattern="[0-5]{1}[0-9]{1}" />:
        <input type="number" class="second" placeholder="<?php esc_attr_e('s', 'ry-invoice-for-amego'); ?>" name="<?php echo esc_attr($field['id']); ?>_second" min="0" max="59" step="1" value="" pattern="[0-5]{1}[0-9]{1}" />
    </p>
    <?php
                    break;
                default:
