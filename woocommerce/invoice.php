...
final class RY_IFAMEGO_WC_Invoice
{
    protected static ?self $_instance = null;

    public static function instance(): RY_IFAMEGO_WC_Invoice
    {
    ...
    {
        $auto_get_statuses = match (RY_IFAMEGO::get_option('get_mode')) {
        ...
        if (RY_IFAMEGO::get_option('invalid_mode') === 'auto_cancel') {
        ...
        add_action('ry_invoice_amego-post_get_invoice', [$this, 'do_get_invoice'], 0, 3);
        add_action('ry_invoice_amego-post_invalid_invoice', [$this, 'do_delete_invoice'], 0, 3);

        if (is_admin()) {
            include_once RY_IFAMEGO_PLUGIN_DIR . 'woocommerce/admin/admin.php';
            RY_IFAMEGO_WC_Admin::instance();

            include_once RY_IFAMEGO_PLUGIN_DIR . 'woocommerce/admin/invoice.php';
            RY_IFAMEGO_WC_Admin_Invoice::instance();
        } else {
            add_filter('default_checkout_invoice_company_name', [$this, 'set_default_invoice_company_name']);
            if (RY_IFAMEGO::get_option('show_invoice_number', 'no') === 'yes') {
    ...
    public function auto_get_invoice($order_ID)
    {
        ...
        if (RY_IFAMEGO::get_option('skip_foreign_order', 'no') === 'yes') {
        ...
        if (!as_has_scheduled_action(RY_IFAMEGO::OPTION_PREFIX . 'auto_get_invoice', [$order->get_id()], 'ry-invoice')) {
            $delay_time = (int) RY_IFAMEGO::get_option('get_delay_time', '0');
            if ($delay_time < 0) {
                $delay_time = 0;
            }
            if ($delay_time > 336) {
            ...
            $order->save();
            as_schedule_single_action(time() + MINUTE_IN_SECONDS * 2 + HOUR_IN_SECONDS * $delay_time, RY_IFAMEGO::OPTION_PREFIX . 'auto_get_invoice', [$order->get_id()], 'ry-invoice');
        }
    }
    ...
    public function auto_delete_invoice($order_ID)
    {
        ...
        if ($invoice_number) {
            switch ($invoice_number) {
                ...
                case 'zero':
                case 'negative':
                    as_unschedule_action(RY_IFAMEGO::OPTION_PREFIX . 'auto_get_invoice', [$order->get_id()], 'ry-invoice');
                    $order->delete_meta_data('_invoice_number');
                    $order->save();
                    break;
                default:
                    as_schedule_single_action(time() + MINUTE_IN_SECONDS * 2, RY_IFAMEGO::OPTION_PREFIX . 'auto_invalid_invoice', [$order->get_id()], 'ry-invoice');
                    break;
            }
        }
    }
    ...
    public function do_get_invoice($post_args, $result, $order_ID)
    {
        ...
        $invoice_date = new DateTime('@' . (string) $result->invoice_time);
        $invoice_date->setTimezone(wp_timezone());

        if (apply_filters('ry_invoice-wc_success_notice', true)) {
            $order->add_order_note(
                __('Invoice number', 'ry-invoice-for-amego') . ': ' . $result->invoice_number . "\n"
                . __('Invoice random number', 'ry-invoice-for-amego') . ': ' . $result->random_number . "\n"
                . __('Invoice create time', 'ry-invoice-for-amego') . ': ' . $invoice_date->format('Y-m-d H:i:s'),
            );
        }

        $order->update_meta_data('_invoice_number', $result->invoice_number);
        $order->update_meta_data('_invoice_random_number', $result->random_number);
    ...
    public function do_delete_invoice($post_args, $result, $order_ID)
    {
        ...
        if (apply_filters('ry_invoice-wc_success_notice', true)) {
            $order->add_order_note(
                __('Invalid invoice', 'ry-invoice-for-amego') . ': ' . $order->get_meta('_invoice_number'),
            );
        }
    ...
    public function get_invoice($order)
    {
        ...
        $invoice_data = [
            ...
            'prefix' => RY_IFAMEGO::get_option('prefix', ''),
            'trackcode' => RY_IFAMEGO::get_option('trackcode', ''),
        ...
        if ($invoice_data['total'] == 0) {
            ...
            $order->save();
            $order->add_order_note(__('Zero total fee without invoice', 'ry-invoice-for-amego'));
            return;
        }
        if ($invoice_data['total'] < 0) {
            $order->update_meta_data('_invoice_number', 'negative');
            $order->save();
            $order->add_order_note(__('Negative total fee can\'t invoice', 'ry-invoice-for-amego'));
            return;
        }
        ...
        if (count($order_items)) {
            foreach ($order_items as $order_item) {
                ...
                $invoice_data['item'][] = [
                    ...
                    'unit' => apply_filters('ry_invoice-item_unit_name', __('parcel', 'ry-invoice-for-amego'), $order->get_id(), $order_item->get_id()),
                ];
            }
        }
        ...
        if (count($fee_items)) {
            foreach ($fee_items as $fee_item) {
                $invoice_data['item'][] = [
                    ...
                    'unit' => apply_filters('ry_invoice-item_unit_name', __('parcel', 'ry-invoice-for-amego'), $order->get_id(), $fee_item->get_id()),
                ];
            }
        }
        ...
        if ($shipping_fee != 0) {
            $invoice_data['item'][] = [
                'name' => __('shipping fee', 'ry-invoice-for-amego'),
                'qty' => 1,
                'total' => round($shipping_fee, wc_get_price_decimals()),
                'unit' => apply_filters('ry_invoice-item_unit_name', __('parcel', 'ry-invoice-for-amego'), $order->get_id(), 'shipping'),
            ];
        }

        if ($total_refunded != 0) {
            $invoice_data['item'][] = [
                'name' => __('return fee', 'ry-invoice-for-amego'),
                'qty' => 1,
                'total' => round(-$total_refunded, wc_get_price_decimals()),
                'unit' => apply_filters('ry_invoice-item_unit_name', __('parcel', 'ry-invoice-for-amego'), $order->get_id(), 'return'),
            ];
        }

        RY_Logs::log('amego-invoice', 'info', 'Get WooCommerce #' . $order->get_id(), $invoice_data);
